...
export interface ParallaxCarouselProps {
  ...
  /** Optional caption under each image. Index matches `images`. */
  renderCaption?: (index: number) => React.ReactNode;
  /** Height reserved below the image for captions (CSS pixels). */
  ...
  (
    ...
    useEffect(() => {
      const tick = (now: number) => {
        ...
        const captions = captionElsRef.current;
        if (captions.length > 0 && count > 0) {
          const stripLength = count * planeStride;
          const halfStrip = stripLength * 0.5;
          for (let index = 0; index < count; index += 1) {
            const el = captions[index];
            if (!el) continue;
            let offsetPx = index * planeStride - s.current;
            if (settings.loop) {
              offsetPx =
    ...
    return (
      ...
      >
        <Canvas
          ...
          style={{ height: captionHeight > 0 ? imageHeight : "100%" }}
        >
          <CameraRig />
          {images.map((src, i) => (
            <Plane
        ...
        {renderCaption && captionHeight > 0 ? (
          <div
            className="pointer-events-none absolute inset-x-0 overflow-hidden"
            style={{ top: imageHeight, height: captionHeight }}
          >
            {images.map((_, i) => (
              <div
                key={`caption-${planeKeys[i]}`}
                ref={(el) => {
                  captionElsRef.current[i] = el;
                }}

...
export default function SoftAurora({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      const w = Math.max(1, container.offsetWidth || container.clientWidth);
      const h = Math.max(1, container.offsetHeight || container.clientHeight);
    ...
    function init() {
      ...
      // Keep CSS aurora visible even when WebGL mounts.
    ...
    return () => {
      ...
      if (renderer?.gl) {
        if (propsRef.current.enableMouseInteraction) {
          renderer.gl.canvas.removeEventListener('mousemove', handleMouseMove);
          renderer.gl.canvas.removeEventListener('mouseleave', handleMouseLeave);
        }

...
const Grainient = ({
  ...
  useEffect(() => {
    ...
    if (Math.floor(rect0.width) < 2 || Math.floor(rect0.height) < 2) return;
    ...
    container.classList.add('grainient-ready');
    ...
    return () => {
      ...
      container.classList.remove('grainient-ready');
      try { container.removeChild(canvas); } catch { /* ignore */ }
      gl.getExtension('WEBGL_lose_context')?.loseContext();
    };
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      <div className="grainient-fallback" />
    </div>
  );
};
